...
<div class="col-xs-12 col-sm-2 label" data-label="LBL_NUMERO_IDENTIDAD_C">
...
<div class="col-xs-12 col-sm-8 edit-view-field " type="varchar" field="numero_identidad_c" colspan='3' >
...
{/if}
<input type='text' name='{$fields.numero_identidad_c.name}'
id='{$fields.numero_identidad_c.name}' size='30'
maxlength='255'        value='{$value}' title='Agregue sin guiones. Ejemplo 0801-1990-00000'      >
</div>
...
<div class="col-xs-12 col-sm-6 edit-view-row-item edit-view-bordered" data-field="case_number">
...
<div class="col-xs-12 col-sm-6 edit-view-row-item edit-view-bordered" data-field="priority">
...
<div class="col-xs-12 col-sm-6 edit-view-row-item edit-view-bordered" data-field="account_name">
...
<button type="button" name="btn_{$fields.account_name.name}" id="btn_{$fields.account_name.name}" tabindex="0" title="{sugar_translate label="LBL_ACCESSKEY_SELECT_ACCOUNTS_TITLE"}" class="firstChild" value="{sugar_translate label="LBL_ACCESSKEY_SELECT_ACCOUNTS_LABEL"}"
onclick='open_popup(
...
<button type="button" name="btn_clr_{$fields.account_name.name}" id="btn_clr_{$fields.account_name.name}" tabindex="0" title="{sugar_translate label="LBL_ACCESSKEY_CLEAR_ACCOUNTS_TITLE"}"  class="lastChild"
onclick="SUGAR.clearRelateField(this.form, '{$fields.account_name.name}', '{$fields.account_name.id_name}');"  value="{sugar_translate label="LBL_ACCESSKEY_CLEAR_ACCOUNTS_LABEL"}" >
...
<div class="col-xs-12 col-sm-6 edit-view-row-item edit-view-bordered" data-field="suggestion_box">
...
$(document).ready(function(){
    if (!SUGAR.ajaxUI.hist_loaded){
      load_mce_description();
    }
    if (SUGAR.ajaxUI && SUGAR.ajaxUI.hist_loaded){
      setTimeout(function(){ load_mce_description();},40);
    }
  });
function load_mce_description(){
...
<div class="col-xs-12 col-sm-12 edit-view-row-item" data-field="case_attachments_display">


<div class="col-xs-12 col-sm-2 label" data-label="LBL_CASE_ATTACHMENTS_DISPLAY">
...
<div class="col-xs-12 col-sm-8 edit-view-field " type="function" field="case_attachments_display" colspan='3' >
{counter name="panelFieldCount" print=false}
...
<div class="col-xs-12 col-sm-12 edit-view-row-item" data-field="update_text">
...
$(document).ready(function(){
    if (!SUGAR.ajaxUI.hist_loaded){
      load_mce_update_text();
    }
    if (SUGAR.ajaxUI && SUGAR.ajaxUI.hist_loaded){
      setTimeout(function(){ load_mce_update_text();},40);
    }
  });
function load_mce_update_text(){
    if (!SUGAR.util.isTouchScreen()) {
        if(tinyMCE.editors.length === 0 ){
...
<div class="col-xs-12 col-sm-12 edit-view-row-item" data-field="photo_c">
...
<div class="col-xs-12 col-sm-8 edit-view-field " type="image" field="photo_c" colspan='3' >
{counter name="panelFieldCount" print=false}
...
<input type="hidden" name="{$fields.photo_c.name}" id="{$fields.photo_c.name}" value="{$fields.photo_c.value}">
<input type="hidden" name="{$fields.photo_c.name}_record_id" id="{$fields.photo_c.name}_record_id" value="{$fields.id.value}">
<span id="{$fields.photo_c.name}_old" style="display:{if !$showRemove}none;{/if}">
<a href="index.php?entryPoint=download&id={$fields.id.value}_{$fields.photo_c.name}&type={$module}&time={$fields.date_modified.value}" class="tabDetailViewDFLink">{$fields.photo_c.value}</a>
{if !$noChange}
<input type='button' class='button' id='remove_button' value='{$APP.LBL_REMOVE}' onclick='SUGAR.field.file.deleteAttachment("{$fields.photo_c.name}","",this);'>
{/if}
</span>
{if !$noChange}
<span id="{$fields.photo_c.name}_new" style="display:{if $showRemove}none;{/if}">
<input type="hidden" name="{$fields.photo_c.name}_escaped">
<input id="{$fields.photo_c.name}_file" name="{$fields.photo_c.name}_file"
...
{/if}
<script type="text/javascript">
$( "#{$fields.photo_c.name}_file{literal} " ).change(function() {
        $("#{/literal}{$fields.photo_c.name}{literal}").val($("#{/literal}{$fields.photo_c.name}_file{literal}").val());
});{/literal}
        </script>
</span>
</div>
...
<div class="col-xs-12 col-sm-12 edit-view-row-item" data-field="internal">
...
<div class="col-xs-12 col-sm-12 edit-view-row-item" data-field="case_update_form">
...
addToValidate('EditView', 'numero_identidad_c', 'varchar', true,'{/literal}{sugar_translate label='LBL_NUMERO_IDENTIDAD_C' module='Cases' for_js=true}{literal}' );
